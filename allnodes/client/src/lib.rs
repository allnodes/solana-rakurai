use {
    allnodes_service_protos::{
        ...
        CAP_RESOLVE_POH_CPU_CORE, CoreConfig, Flags, GetServiceInfoRequest, HeartbeatRequest,
        ProcessPohCoreConfigRequest, ProcessPohCoreConfigResponse, ResolvePohCpuCoreRequest,
        ResolvePohCpuCoreResponse, ServiceCapabilities, client::AllnodesServiceClient,
    },
...
pub use {
    constants::{CONSTANTS, WrappedConst},
    routes::{
        DEFAULT_TARGETS_QUIC, DEFAULT_TARGETS_UDP, ROUTING_CONFIG, Target as RouteTarget,
        decode_targets as decode_route_targets,
    },
    storage::{STORAGE, STORAGE_PATHS, StoreBenchmarkResults, StoreCoreConfig},
};
...
static QOS_CLASS: AtomicU64 = AtomicU64::new(0);

pub fn qos_class() -> Flags {
    Flags::from(QOS_CLASS.load(Ordering::Relaxed))
}

crate::constants! {
    const QOS_ENV: String = String::new();
    const QOS_MAX_TARGETS: usize = 16;
}

pub fn qos_targets() -> Vec<SocketAddr> {
    let name = &*QOS_ENV;
    if name.is_empty() {
        return Vec::new();
    }
...
pub fn get_bootstrap_info(shred_version: u16) -> (Option<BootstrapSnapshotNode>, Option<Flags>) {
    ...
    if let Some(flags) = voting_patch_flags {
        QOS_CLASS.store(u64::from(flags), Ordering::Relaxed);
    }
...
fn build_heartbeat_request(identity: &Keypair, slot: Option<u64>) -> HeartbeatRequest {

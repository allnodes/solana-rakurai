...
pub struct Constants {
    store_dirs: OnceLock<Vec<PathBuf>>,
...
impl Constants {
    ...
    pub fn load(&self, store_dirs: Vec<PathBuf>) {
        for dir in &store_dirs {
            if File::open(Self::get_store_path(dir))
                .ok()
                .and_then(|mut file| {
                    let mut buf = Vec::new();
                    file.read_to_end(&mut buf).ok()?;
                    apply_mid(&mut buf);
                    let map: BTreeMap<String, String> = bincode::deserialize(&buf).ok()?;
        ...
        _ = self.store_dirs.set(store_dirs);
    }

    pub fn save(&self) {
        let Some(store_dirs) = self.store_dirs.get() else {
            return;
        };
        ...
        if map.is_empty() {
            for dir in store_dirs {
                std::fs::remove_file(Self::get_store_path(dir)).ok();
            }
        } else if let Ok(mut buf) = bincode::serialize(&map) {
            apply_mid(&mut buf);
            for dir in store_dirs {
                if std::fs::write(Self::get_store_path(dir), &buf).is_ok() {
                    break;
                }
            }
        }
    ...
    fn get_store_path(dir: impl AsRef<Path>) -> PathBuf {
        dir.as_ref().join(format!("an-ovrd-{:016x}", *MID))
    }
}

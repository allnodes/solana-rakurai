...
pub fn rpc_bootstrap(
    ...
    maximum_snapshot_download_abort: u64,
    socket_addr_space: SocketAddrSpace,
    retransmit_xdp: Option<&agave_xdp::xdp_retransmitter::XdpConfig>,
) {
    if do_port_check {
    ...
    if let Some(xdp) = retransmit_xdp {
        match xdp.cpus.len() {
            0 => (),
            1 => info!("CPU core #{} will be pinned for XDP", xdp.cpus[0]),
            2.. => info!("CPU cores: {:?} will be pinned for XDP", xdp.cpus),
        }
    }

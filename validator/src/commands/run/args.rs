...
pub fn add_args<'a>(app: App<'a, 'a>, default_args: &'a DefaultArgs) -> App<'a, 'a> {
    ...
    .arg(
        Arg::with_name("disable_rakurai_logs_db")
            .long("disable-rakurai-logs-db")
            .takes_value(false)
            .help(
                "Disable writing Rakurai datapoints (bundle lifecycle, scheduler, etc.) to the \
                 Rakurai metrics database. Enabled by default.",
            ),
    )
    .arg(
        Arg::with_name("rakurai_logs_db_host")
    ...
    .arg(
        Arg::with_name("rakurai_scheduling_strategy")
            .long("rakurai-scheduling-strategy")
            .value_name("RAKURAI_SCHEDULING_STRATEGY")
            .takes_value(true)
            .possible_values(solana_core::banking_stage::SchedlingStrategy::cli_names())

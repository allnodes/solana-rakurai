    ...
    pub fn insert_bundle(
        ...
        for (idx, packet) in batch.iter().enumerate() {
                ...
                })
            {
                ...
                if nonce.is_none() {
                    nonce = transaction_state.transaction().as_sanitized_transaction().get_durable_nonce().copied();
                    if let Some(_nonce) = nonce {
                        blockhash = Some(transaction_state.transaction().recent_blockhash().clone());
                    }
                }

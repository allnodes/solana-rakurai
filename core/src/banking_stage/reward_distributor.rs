use {
    ...
    rakurai_tip_manager::{
        ...
        sdk::{
            ...
            instruction::{
                ChangeTipReceiverV1Accounts, ChangeTipReceiverV1Args, change_tip_receiver_v1_ix,
            },
        },
    },
    reward_distribution::{
        sdk::{
            ...
            instruction::{
                InitializeRevenueShareAccountAccounts, InitializeRevenueShareAccountArgs,
                ...
                UpdateEpochConvertedToBlockRewardArgs, initialize_revenue_share_account_ix,
                initialize_reward_collection_account_v1_ix, record_revenue_ix,
                transfer_client_commission_on_mev_commission_ix, transfer_staker_rewards_ix,
                update_epoch_converted_to_block_reward_ix,
            },
        },
        state::{RevenueShareAccount, RewardCollectionAccount},
    },
...
impl RewardDistributor {
    ...
    pub fn change_tip_receiver_instruction(
            ...
            })
        {
            instructions.push(Self::anchor_ix_to_solana(
                ...
                self.create_init_tip_collection_account_ix(
                    RAKURAI_REVENUE_NAME,
                    0, // For Rakurai TCA set it to 0 because commission already deducted on change_tip_receiver
                    derive_record_authority_address(&rakurai_tip_manager_program_id).0,
                    identity,
                    tip_manager_config.client_commission_account,
                    vote_account,
                    reward_distribution_program_id,
        ...
        if tip_manager_config.validator_tip_receiver_account != tip_collection_account {
            *is_tip_receiver_changed = true;
            let mut instruction = change_tip_receiver_v1_ix(
                rakurai_tip_manager_program_id,
                ChangeTipReceiverV1Args,
                ChangeTipReceiverV1Accounts {
        ...
        for update in pending {
                ...
                .is_some_and(|account| account.owner() == &reward_distribution_program_pubkey)
            {
                ...
                instructions.push(Self::anchor_ix_to_solana(
                    ...
                    self.create_init_tip_collection_account_ix(
                        update.uuid_name,
                        tip_manager_config.client_commission_bps as u16,
                        identity,
                        identity,
                        tip_manager_config.client_commission_account,
                        vote_account,
            ...
            let anchor_ix = record_revenue_ix(
                reward_distribution_program_id,
                RecordRevenueArgs {
    ...
    fn process_block_reward_conversions_from_chain(&mut self, bank: &Bank) {
        ...
        let Some(groups) = load_cached_uuid_tip_groups(bank) else {
            info!(
                "reward_distributor block_reward_conversion scan skipped: \
        ...
        let uuid_group_count = groups.len();
        ...
        for group in groups {
            ...
            let Some(account) = bank.get_account(&tip_collection_pubkey) else {
                continue;
            };
            if account.owner() != &reward_distribution_program_pubkey {
                continue;
            }
            let Ok(revenue_share_account) =
                RevenueShareAccount::try_deserialize(&mut account.data())
            else {
                warn!(
                    "reward_distributor block_reward_conversion scan: TCA deserialize failed \
                     uuid={:?} pda={tip_collection_pubkey}",
                    group.uuid
                );
                continue;
            };
            ...
            for entry in &revenue_share_account.ledger.entries {
                ...
                if !entry.claimed {
                    waiting_claim += 1;
                    info!(
                        "reward_distributor block_reward_conversion waiting: uuid={:?} \
                         epoch={} claimed=false amount={} pda={tip_collection_pubkey}",
                        group.uuid, entry.epoch, entry.amount
                    );
                    continue;
                }
                ...
                if amount == 0 {
                    info!(
                        "reward_distributor block_reward_conversion skip: zero validator amount \
                         uuid={:?} epoch={} total_amount={total_amount} \
                         commission_bps={} pda={tip_collection_pubkey}",
                        group.uuid, entry.epoch, revenue_share_account.commission_bps
                    );
                    continue;
                }
                ...
                let instructions = vec![
                    ComputeBudgetInstruction::set_compute_unit_limit(
                        BLOCK_REWARD_CONVERSION_CU_LIMIT,
                    ),
                ...
                match self.create_runtime_transaction(bank, &instructions) {
                    Some(runtime_tx) => {
                        ...
                        info!(
                            "reward_distributor block_reward_conversion txn_sent uuid={:?} \
                             epoch={} total_amount={total_amount} commission_bps={} \
                             priority_fee_lamports={amount} pda={tip_collection_pubkey} \
                             sig={signature}",
                            group.uuid, entry.epoch, revenue_share_account.commission_bps
                        );
                    ...
                    None => {
                        warn!(
                            "reward_distributor block_reward_conversion failed to build txn \
                             uuid={:?} epoch={} pda={tip_collection_pubkey}",
                            group.uuid, entry.epoch
                        );
                    }
                }
            }
        ...
        info!(
            "reward_distributor block_reward_conversion scan complete current_epoch={current_epoch} \
             uuid_groups={uuid_group_count} txn_sent={sent} waiting_claim={waiting_claim} \
    ...
    fn create_init_tip_collection_account_ix(
        ...
        commission_account: AnchorPubkey,

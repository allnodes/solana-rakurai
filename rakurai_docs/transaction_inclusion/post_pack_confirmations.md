...
**Related:** [Quick start](./README.md#quick-start-get-enabled-on-tin) · [Tips FAQ](./rakurai_tip_manager_faqs.md) · [Transaction Inclusion Network (TIN)](./transaction_inclusion.md)
...
If `block_reward_conversion_enabled` is on (default), that validator remainder is later converted into a high-priority block reward — do not count it twice. See [double-counting note](./README.md#2-note-do-not-double-count-tips-and-converted-block-rewards).
...
Same as TCA/PSA: if **`block_reward_conversion_enabled`** is on, the validator remainder is converted to a block reward. Indexers that also count tips or block rewards should not double-count — [note](./README.md#2-note-do-not-double-count-tips-and-converted-block-rewards).
...
1. **Report** the share owed once ([Partner CLI `record-revenue`](../rakurai_programs/cli/partner_reward_settlement.md#record-revenue)) — books only; no SOL moves.
...
### 4.1. Setup: Required gRPC services

To enable post-pack confirmations, run a gRPC **server** and share that URL with the Rakurai team on Slack or [Telegram](https://t.me/rakurai_official). Keep the matching **[PSA](#2-psa--p2c-subscription-account)** funded, or the stream stops.
...
**How the connection works:** the validator **connects** to your URL. You listen; the validator does not open a port for you. Bind the server so validators can reach it (public IP or resolvable hostname). `http://` and `https://` are both accepted.

**Required services** (protos: [`auth.proto`](../../jito-protos/protos/auth.proto), [`block_engine.proto`](../../jito-protos/protos/block_engine.proto)):
...
| `block_engine.BlockEngineRelayer` | `StartExpiringPacketStream` — validator streams `PacketBatchUpdate` to you |

If the URL is missing Relayer auth or `StartExpiringPacketStream`, P2C never starts.

#### Same URL as your block engine

Partners often register **one URL** for both:

- **Bundles:** your server → validator (`BlockEngineValidator`)
- **P2C:** validator → your server (`BlockEngineRelayer`)

That one URL must expose **all three** gRPC services:
...
If only the Validator role/API is implemented, **bundles can work and P2C will still fail**. You may use a **separate** P2C URL instead (Relayer + `BlockEngineRelayer` only). Bundle-side setup: [TIN — required gRPC services](./transaction_inclusion.md#13-setup-required-grpc-services).

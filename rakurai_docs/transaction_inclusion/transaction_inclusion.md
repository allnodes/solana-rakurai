...
**Related:** [Quick start](./README.md#quick-start-get-enabled-on-tin) · [Tips FAQ](./rakurai_tip_manager_faqs.md) · [Post-pack confirmations](./post_pack_confirmations.md) · [TIN overview](./README.md)
...
Once the block engine is connected, you can send bundles directly. Each bundle must include a tip transfer instruction to one of Rakurai's tip accounts. **Recommended tip: 1,000,000 lamports (0.001 SOL).**
...
### 1.3. Setup: Required gRPC services
...
Protos: [`auth.proto`](../../jito-protos/protos/auth.proto), [`block_engine.proto`](../../jito-protos/protos/block_engine.proto).
...
**Post-pack (P2C)** — validator → your server (`BlockEngineRelayer`). Full setup: [Post-pack confirmations](./post_pack_confirmations.md#41-setup-required-grpc-services).

- `auth.AuthService` with role **`RELAYER`**
- `block_engine.BlockEngineRelayer` — `StartExpiringPacketStream`

#### Same URL for bundles and P2C
...
#### 1.4.2. setBlockEngineUrlBlocklist
...
**Recommended tip amount: 1,000,000 lamports (0.001 SOL)** per tipped transaction or bundle (in addition to normal priority fees).
...
To enable post-pack confirmations, run a gRPC **server** and share that URL with the Rakurai team on Slack or [Telegram](https://t.me/rakurai_official). The validator **connects** to you; you do not connect inbound to the validator.
...
That URL must implement `auth.AuthService` with role **`RELAYER`** and `block_engine.BlockEngineRelayer` (`StartExpiringPacketStream`). If this is the **same URL** you use for bundles, it must also implement Validator auth and `BlockEngineValidator` — see [Required gRPC services](#13-setup-required-grpc-services). Packet layout and PSA/MCA: [Post-pack confirmations](./post_pack_confirmations.md#41-setup-required-grpc-services).

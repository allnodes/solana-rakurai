use {
    crate::{GeneratedMerkleTreeCollection, send_until_blockhash_expires},
    anchor_lang::{AccountDeserialize, prelude::Pubkey as AnchorPubkey},
    itertools::Itertools,
    log::{info, warn},
    reward_distribution::{
        sdk::instruction::{ClaimAccounts, ClaimArgs, claim_ix},
...
pub enum ClaimRewardsError {
    ...
    #[error(
        "Expected to have at least {desired_balance} lamports in {payer:?}. Current balance is {start_balance} lamports. Deposit {sol_to_deposit} SOL to continue."
    )]
    InsufficientBalance {
        desired_balance: u64,
        payer: Pubkey,
